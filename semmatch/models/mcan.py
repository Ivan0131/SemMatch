...
class MCAN(Model):
    ...
    def forward(self, features, labels, mode, params):
        ...
        with tf.variable_scope(self._model_name):
            ...
            query_output = query_emb #nn.highway_network(query_emb, 1, dropout_rate=self._drop_rate, is_trainging=is_training,
                                 #             scope="query_highway")
            doc_output = doc_emb #nn.highway_network(doc_emb, 1, dropout_rate=self._drop_rate, is_trainging=is_training,
                                 #           scope="doc_highway")
            ...
            query_final = tf.concat([query_mean], axis=1)
            ...
            doc_final = tf.concat([doc_mean], axis=1)
