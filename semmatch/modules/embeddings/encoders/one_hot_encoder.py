...
class OneHotEncoder(Encoder):
    def __init__(self, n_values, on_value=1.0, off_value=0.0, axis=-1, dtype=tf.float32, encoder_name='one_hot_encoder'):
    ...
    def forward(self, features, labels, mode, params):
        with tf.variable_scope(self._encoder_name):
            output = tf.one_hot(features, depth=self._n_values, on_value=self._on_value, off_value=self._off_value,
